...
Процедура ПодготовитьИОтправитьПЭДПродолжить(Результат, ДопПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатОтправкиПЭД.МассивПакетовДляПовторнойОтправки) Тогда
		ТекстСообщения = НСтр("ru = 'Истек срок действия маркера доступа.
			|Выполняется повторная авторизация и отправка пакетов электронных документов.'");
...
Процедура ОтправитьПолучитьОбменСКонтрагентами(ТребуетсяПовторитьПолучение = Ложь) Экспорт
	...
	Если ПараметрыОтправкиПолученияЭД.Свойство("ПрофилиЭДОБезСертификатов") Тогда
		ШаблонСообщения = НСтр("ru = 'В профиле ЭДО %1 нет доступных сертификатов электронной подписи.
			|Для отправки и получения электронных документов по данному профилю ЭДО необходимо добавить действительный личный сертификат.'");
		Для Каждого ПрофильЭДО Из ПараметрыОтправкиПолученияЭД.ПрофилиЭДОБезСертификатов Цикл
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ПрофильЭДО);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ПрофильЭДО, "СертификатыПодписейОрганизации");
		КонецЦикла;
	КонецЕсли;
