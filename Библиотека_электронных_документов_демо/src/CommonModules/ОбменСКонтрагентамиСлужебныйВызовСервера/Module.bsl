...
Процедура ОбновитьВерсиюЭД(ЭлектронныйДокумент, ПринудительнаяСменаСостоянияВерсии = Ложь, ВерсияФорматаПакета = Неопределено, 
	...
	Если ЗначениеЗаполнено(НовоеСостояниеВерсииЭД) Тогда
		...
		Если НовоеСостояниеВерсииЭД <> ТекущееСостояниеВерсииЭД Тогда
			...
			НаОзнакомление = ОбменСКонтрагентамиСлужебный.ЕстьНеобходимостьОзнакомленияСИзменениемСостоянияЭД(
	...
	Для каждого ПакетЭД Из МассивПакетовЭД Цикл
		...
		Попытка
			...
			Если Не ОбменСКонтрагентамиВнутренний.РаспаковатьАрхив(
				ФайлАрхива, ПапкаДляРаспаковки, НСтр("ru = 'Распаковка архива пакета ЭД'")) Тогда
...
Функция ТекстСостоянияЭД(СсылкаНаВладельца, Гиперссылка) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда
		...
		Если НастройкаЭДСуществует(СсылкаНаВладельца, ПараметрыЭД) Тогда
			Если Не ОбменСКонтрагентамиСлужебныйВызовСервера.ЕстьПравоЧтенияЭД() Тогда
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда
		...
		Если ПолучаемДанные Тогда
			...
			ИначеЕсли СтрНайти(ПараметрыФайлаЭД.Расширение, "zip") > 0 Тогда
				...
				ОбменСКонтрагентамиВнутренний.РаспаковатьАрхив(
					ИмяАрхива, ПапкаДляРаспаковки, НСтр("ru = 'Извлечение ЭД CML из архива при подписи'"));
...
Функция ВыполнитьДействияПоЭД(Знач МассивСсылокНаОбъект,
	...
	Если ЭД = Неопределено И ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Сформировать") Тогда
		...
		МассивОбработанныхСФПоОснованиям = Неопределено;
		Если КолНовыхЭД > 0 И ДопПараметры.Свойство("МассивОбработанныхСФПоОснованиям", МассивОбработанныхСФПоОснованиям)
			И ЗначениеЗаполнено(МассивОбработанныхСФПоОснованиям) Тогда
			Счетчик = ТЗ_ЭД_СФ.Количество() - 1;
			Пока Счетчик > -1 Цикл
				СтрокаТаблицы = ТЗ_ЭД_СФ[Счетчик];
				Если МассивОбработанныхСФПоОснованиям.Найти(СтрокаТаблицы.ВладелецЭД) <> Неопределено Тогда
		...
		Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Продавец
			...
			Иначе
				...
				Если ЗначениеЗаполнено(ОтчествоСертификата) Тогда
					ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Отчество", ОтчествоСертификата, , Ошибки);
				КонецЕсли;
				ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(ОтпускРазреш, "ФИО", ФИО, Истина, Ошибки);
				ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(ОтпускРазреш, "Должность", ДолжностьПоСертификату, , Ошибки);
				ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.СвТНО.ОтпускГруз, "ОтпускРазреш", ОтпускРазреш, Истина, Ошибки);
		...
		ИначеЕсли ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.АктИсполнитель Тогда
			...
			Если Не ВРег(ПространствоИменСхемы) = ВРег("RUISP") Тогда
				...
				Если ЗначениеЗаполнено(ОтчествоСертификата) Тогда
					ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Отчество", ОтчествоСертификата, , Ошибки);
				КонецЕсли;
				ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(ПодписьИсполн, "ФИО", ФИО, Истина, Ошибки);
				ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(ПодписьИсполн, "Должность", ДолжностьПоСертификату, Истина, Ошибки);
				ОбменСКонтрагентамиВнутренний.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.СвАктИ.Сдал, "ПодписьИсполн", ПодписьИсполн, Истина, Ошибки);
...
Процедура СброситьСвойствоXDTO(ОбъектXDTO, ИмяСвойства, Ошибки)
	...
	Исключение
		...
		ЭлектронноеВзаимодействиеСлужебныйКлиентСервер.ДобавитьОшибку(Ошибки, ТекстСообщения);
...
Функция НемедленнаяОтправкаЭД() Экспорт
	...
	Настройки = ОбменСКонтрагентамиСлужебный.НастройкиОбменаСКонтрагентами();
