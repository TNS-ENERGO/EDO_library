...
Процедура НайтиТорговыеПредложенияПоОтбору(Знач ПараметрыПоиска, Знач АдресРезультата) Экспорт
	...
	Если ПараметрыПоиска.Свойство("Поставщик") И ЗначениеЗаполнено(ПараметрыПоиска.Поставщик) Тогда
		...
		Отказ = Ложь;
		Идентификаторы = БизнесСетьКлиентСервер.ИдентификаторыУчастника(РеквизитыПоставщика.ИНН, РеквизитыПоставщика.КПП,, Отказ);
		Если Отказ Тогда
			Возврат;
		КонецЕсли;;
...
Функция ПолучитьПодсказкуПоКонтексту(ДанныеКонтекста, Знач АдресРезультата) Экспорт
	...
	Если ДанныеКонтекста.Организации.Количество() Тогда
		...
		Для каждого Организация Из ДанныеКонтекста.Организации Цикл
			...
			Отказ = Ложь;
			Идентификаторы = БизнесСетьКлиентСервер.ИдентификаторыУчастника(РеквизитыОрганизации.ИНН, РеквизитыОрганизации.КПП,, Отказ);
			Если Отказ Тогда
				Продолжить;
			КонецЕсли;
...
Функция ПараметрыКомандыСопоставитьЗначенияРеквизитов(ПараметрыМетода, Отказ = Неопределено) Экспорт
	...
	ОбъектПоискаЗначений = ФабрикаXDTO.Создать(ТорговыеПредложения.ТипXDTO("AttributeValueSearch"));
	
	Для каждого ЗначениеДляСопоставления Из ПараметрыМетода.Данные Цикл 
		ОбъектЗначения = ФабрикаXDTO.Создать(ТорговыеПредложения.ТипXDTO("AttributeValue"));
	...
	Данные = ТорговыеПредложения.ОбъектXDTOВСтрокуJSON(ОбъектПоискаЗначений, -1);
...
Функция ПараметрыКомандыПолучитьКоличествоТорговыхПредложений(ПараметрыМетода, Отказ = Неопределено)
	...
	Идентификаторы = БизнесСетьКлиентСервер.ИдентификаторыУчастника(ПараметрыМетода.ИНН, ПараметрыМетода.КПП,, Отказ);
	Если Отказ Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПараметрыКомандыСоздатьТорговоеПредложение(ПараметрыМетода, Отказ)
	...
	ТекстОшибки = "";
	Идентификаторы = БизнесСетьКлиентСервер.ИдентификаторыУчастника(ПараметрыМетода.ИНН, ПараметрыМетода.КПП,, Отказ, ТекстОшибки);
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		ЭлектронноеВзаимодействиеСлужебный.ВыполнитьЗаписьСобытияПоЭДВЖурналРегистрации(
			НСтр("ru = 'Ошибка публикации торговых предложений:'") + " " + ТекстОшибки, "БизнесСеть");
...
Функция СформироватьПакетПредложенийXDTO(Знач ПараметрыТорговогоПредложения, ТаблицаКартинок, КоличествоПозиций, Отказ)
	...
	ТекстОшибки = "";
	ИдентификаторыОрганизации = БизнесСетьКлиентСервер.ИдентификаторыУчастника(ПакетПредложений.КонтактыОрганизации.ИНН,
	...
	Для каждого СтрокаКонтрагентов Из ПакетПредложений.СписокКонтрагентов Цикл
		...
		ИдентификаторыКонтрагента = БизнесСетьКлиентСервер.ИдентификаторыУчастника(СтрокаКонтрагентов.ИНН,
