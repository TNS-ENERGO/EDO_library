...
Процедура ОбменСКонтрагентамиВладелецПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	ДанныеСостояния = ОбменСКонтрагентамиСлужебный.ДанныеСостоянияЭД(Источник.Ссылка);
				...
				Если НастройкиОбмена.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
					...
					Если НастройкиОбмена.Направление = Перечисления.НаправленияЭД.Исходящий Тогда
						...
						Иначе	
							...
							ИначеЕсли РеквизитыВладельцаЭД.СостояниеЭДО = Перечисления.СостоянияВерсийЭД.ОжидаетсяКорректировка Тогда
...
Функция СформироватьПрисоединенныеФайлы(МассивОбъектов, ПараметрыОбмена, ДопПараметры = "") Экспорт
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если СтруктураОбмена.Свойство("СчетФактураОснования")
			И ЗначениеЗаполнено(СтруктураОбмена.СчетФактураОснования) Тогда
			
			Если ДопПараметры = "" Тогда
				ДопПараметры = Новый Структура;
			КонецЕсли;
			
			Если ДопПараметры.Свойство("МассивОбработанныхСФПоОснованиям") Тогда
				ДопПараметры.МассивОбработанныхСФПоОснованиям.Добавить(СтруктураОбмена.СчетФактураОснования);
			Иначе
				ДопПараметры.Вставить("МассивОбработанныхСФПоОснованиям",
					ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтруктураОбмена.СчетФактураОснования));
			КонецЕсли;
		...
		Если СтрокаДерева.СсылкаНаОбъект <> Контрагент Тогда
			...
			Если ОбменСКонтрагентамиПовтИсп.ИспользуетсяДополнительнаяАналитикаКонтрагентовСправочникПартнеры() Тогда
				...
				Если СтрокаДереваПартнер <> Неопределено Тогда
					...
					СтрокаДереваПартнер.СсылкаНаОбъект = ЭлектронноеВзаимодействиеСлужебныйВызовСервера.НайтиСсылкуНаОбъект(ИмяПрикладногоСправочника, , РеквизитыПартнера);
				КонецЕсли; 
			КонецЕсли;
		КонецЕсли;
...
//
Функция ЕстьНеобходимостьОзнакомленияСИзменениемСостоянияЭД(Знач ТекущееСостояние, Знач НовоеСостояние) Экспорт
...
Процедура СоздатьФайлыОписанияПакета(РеквизитыПакета)
	...
	// Фамилия.
	ЭДКарточки.ЗаписатьНачалоЭлемента("AdditionalParameter");
	ЭДКарточки.ЗаписатьАтрибут("Name",  "AbonentSirname");
	...
	// Телефон.
	ЭДКарточки.ЗаписатьНачалоЭлемента("AdditionalParameter");
	ЭДКарточки.ЗаписатьАтрибут("Name",  "AbonentPhone");
	ЭДКарточки.ЗаписатьАтрибут("Value", РеквизитыПакета.Телефон);
	ЭДКарточки.ЗаписатьКонецЭлемента();
