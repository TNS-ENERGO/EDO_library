...
//  Назначение - Строка - назначение операции для добавления текста в предупреждение установки внешней компоненты.
//
Процедура ВыполнитьБазовуюАутентификациюСбербанк(Оповещение, ИмяВнешнегоМодуля, КлючСессии, НастройкаОбмена = Неопределено, ЛогинПароль = Неопределено, Назначение = Неопределено) Экспорт
	...
	ПодключитьВнешнююКомпонентуСбербанка(ОповещениеПослеПодключенияВК, ИмяВнешнегоМодуля, Назначение);
...
Процедура ПодключитьВнешнююКомпонентуСбербанка(Оповещение, ИмяМодуля, Назначение = Неопределено)
	...
	Если Назначение = "НастройкаОбмена" Тогда
		ТекстУточнения = НСтр("ru = 'При отказе от установки подключение обмена будет невозможно'");
	КонецЕсли;
	
	ДополнительныеВнешниеКомпонентыКлиент.ПодключитьВнешнююКомпонентуПоИдентификатору(
		ОповещениеПослеПодключенияВК, ИмяМодуля, Ложь, ТекстУточнения); // версию не контролируем, т.к есть ошибка в ВК Сбербанка (лишний символ)
...
Процедура ОтправкаСообщенийОбмена(Параметры)
	...
	Если Результат.Свойство("СтруктураКОтправке", СтруктураКОтправке) Тогда
		...
		Иначе
			...
			Если МассивСообщенийОбменаКУдалениюИзОтправки.Количество() > 0 И СтруктураКОтправке.СПодписью.Количество() > 0 Тогда
				Для Каждого УдаляемыйЭД Из МассивСообщенийОбменаКУдалениюИзОтправки Цикл
					ТекИндекс = СтруктураКОтправке.СПодписью.Найти(УдаляемыйЭД);
					Если ТекИндекс <> Неопределено Тогда
						СтруктураКОтправке.СПодписью.Удалить(ТекИндекс);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		
			Параметры.Вставить("МассивКОтправке", СтруктураКОтправке.СПодписью);
			...
			Если Структура.Свойство("СообщенияОбменаИПрисоединенныеФайлы", СообщенияОбменаИПрисоединенныеФайлы) Тогда
				...
				Иначе
					...
					Для Каждого Элемент Из СписокПредставленийСообщенийОбмена Цикл
						ОбработчикОткрытияЭД = Новый ОписаниеОповещения("ПриОткрытииЭлектронногоДокумента", ЭтотОбъект, Элемент.Ключ);
						СтруктураПредставления = Новый Структура("Представление, Значение", Элемент.Значение, ОбработчикОткрытияЭД);
...
Процедура ПолучитьСвойстваСертификатовПослеПодключенияВК(Результат, ДополнительныеПараметры) Экспорт
	...
	ПолучитьСвойстваСертификатовСообщенийОбменаЧерезВК(Оповещение, ПодключаемыйМодуль,
		ДополнительныеПараметры.МассивСообщенийОбмена, ДополнительныеПараметры.НастройкаОбмена);
