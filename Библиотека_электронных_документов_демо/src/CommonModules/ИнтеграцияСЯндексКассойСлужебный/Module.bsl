...
Процедура ПроверитьОрганизациюНаСоответствиеТребованиямПриПодключенииКСервису(Знач Организация,
	...
	ЗначенияРеквизитов = ИнтеграцияСЯндексКассойСлужебный.РеквизитыОрганизации(Организация);
...
//
// Возвращаемое значение:
//  Строка - ссылка на страницу оплаты.
//
Функция ПлатежнаяСсылка(Знач ВходящиеПараметры) Экспорт 
	
	// &ЗамерПроизводительности
	ОписаниеЗамера = ЭлектронноеВзаимодействиеСлужебный.НачатьЗамерВремени("ИнтеграцияСЯндексКассой.ФормированиеПлатежнойСсылки");
	...
	ПараметрыСервиса = ПараметрыСервиса(ИмяКоманды, ВходящиеПараметры, Отказ);
	...
	Результат = РезультатРазбора.ДанныеXDTO.OrderURL;
...
Функция ОперацииПоЯндексКассе(Знач ВходящиеПараметры) Экспорт
	...
	// &ЗамерПроизводительности
	ОписаниеЗамера = ЭлектронноеВзаимодействиеСлужебный.НачатьЗамерВремени("ИнтеграцияСЯндексКассой.ПолучениеОперацийПоЯндексКассе");
	КоличествоПовторяющихсяДанных = 0;
	...
	// &ЗамерПроизводительности
	ЭлектронноеВзаимодействиеСлужебный.ЗафиксироватьЗамерВремени(ОписаниеЗамера);
	...
	КонецЦикла;
	// &ЗамерПроизводительности
	ЭлектронноеВзаимодействиеСлужебный.ЗафиксироватьЗамерВремени(ОписаниеЗамера, КоличествоПовторяющихсяДанных);
...
Функция ВыполнитьМетодСервисаВФоне(Знач ИмяМетода, Знач ВходящиеПараметры, Знач ИдентификаторФормы) Экспорт
	...
	Если ИмяМетода = "ДанныеОснованияПлатежаВСервисе" Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение данных основания платежа в сервисе Яндекс.Касса.'");
		
	ИначеЕсли ИмяМетода = "ПлатежнаяСсылка" Тогда
		
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Получение ссылки для оплаты через Яндекс.Кассу.'");
...
Процедура ОбновитьНастройкуЯндексКассы(НастройкаСсылка, Параметры, Отказ)
	...
	Если Не ПравоДоступа("Изменение", Метаданные.Справочники.НастройкиЯндексКассы) Тогда
		Возврат;
	КонецЕсли;
...
Функция НастройкиСервиса()
	...
	ЗаполнитьЗначенияСвойств(ПараметрыСоединения, ОбщиеНастройкиСервиса().СервисУведомлений);
...
Функция ДанныеОснованияПлатежаДляСервиса_Чек(Знач ДанныеОснованияПлатежа) 
	...
	КонтактныеДанныеЧека = ДанныеОснованияПлатежа.Покупатель.КонтактныеДанныеЧека;
	
	Чек.Вставить("customerContact", ?(КонтактныеДанныеЧека = Неопределено, "NULL", КонтактныеДанныеЧека));
...
Функция ДанныеОснованияПлатежаВСервисе(Знач ВходящиеПараметры)
	...
	ИначеЕсли РезультатРазбора.ДанныеXDTO.Status = 0 И РезультатРазбора.ДанныеXDTO.OrderState <> 2 Тогда
...
Функция ПараметрыСервиса(Знач ИмяКоманды, Знач ВходящиеПараметры, Отказ = Ложь) 
	...
	Параметры.Вставить("ДанныеАутентификации",
		"Basic " + СтрокаBase64БезBOM(ДанныеАутентификации.Логин + ":" + ДанныеАутентификации.Пароль));
...
Процедура ВыполнитьКомандуСервиса(Знач ИмяКоманды, ПараметрыКоманды, Результат, Отказ)
...
Функция ТекстыСообщенийОбОшибках(ИдентификаторОшибки, ДополнительныеПараметры = Неопределено)
	...
	ИначеЕсли ИдентификаторОшибки = "НеНайденаДействительнаяНастройка" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			ВидОперацииПоКоманде(ДополнительныеПараметры.ИмяКоманды),
			НСтр("ru = 'Не найдена действующая настройка Яндекс.Кассы, формирование ссылки на страницу оплаты через Яндекс.Кассу не возможно'"),
	...
	ИначеЕсли ИдентификаторОшибки = "НеНайденыДействующиеНастройки" Тогда
		...
		Сообщение = СообщениеОбОшибке(
			ВидОперацииПоКоманде(ДополнительныеПараметры.ИмяКоманды),
			НСтр("ru = 'Не найдены действующие настройки Яндекс.Кассы.'"),
...
Функция ДанныеАутентификацииИПП()
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Результат.Пароль = ДанныеБилетаИПП.Тикет;
