...
//
Функция ОбработчикСобытияБизнесПроцесса(КонтекстВзаимодействия, ИмяСобытия) Экспорт
...
Процедура ПередОткрытиемФормы(
	...
	Если ДопОбработчик <> Неопределено Тогда
		...
		ДопОбработчик.ПередОткрытиемФормы(
			ИмяОткрываемойФормы,
			ПараметрыОткрытияФормы,
			КонтекстВзаимодействия,
			КонтекстОбработчика,
...
//  Логин - Строка - логин для авторизации на портале поддержи пользователей;
//
Процедура ОткрытьЛичныйКабинетПользователя()
