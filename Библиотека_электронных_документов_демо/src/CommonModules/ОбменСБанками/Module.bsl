...
Процедура ПолучитьДанныеВыпискиБанкаТекстовыйФормат(Знач СообщениеОбмена, СсылкаНаХранилище, МассивСчетов = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаСоздания) Тогда
		Текст.ЗаписатьСтроку("ДатаСоздания=" + Формат(ДатаСоздания, "ДФ=dd.MM.yyyy"));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВремяСоздания) Тогда
		Текст.ЗаписатьСтроку("ВремяСоздания=" + Формат(ВремяСоздания, "ДФ=чч:мм:сс"));
	КонецЕсли;
	...
	Текст.ЗаписатьСтроку("ДатаНачала=" + Формат(ДатаНачала, "ДФ=dd.MM.yyyy"));
	Текст.ЗаписатьСтроку("ДатаКонца="  + Формат(ДатаКонца,  "ДФ=dd.MM.yyyy"));
	...
	Для Каждого СтруктураДанных Из МассивДанных Цикл
		...
		Для Каждого СтрокаТЧ Из СтрокиТЧ Цикл
			...
			Текст.ЗаписатьСтроку("Дата=" + Формат(Дата, "ДФ=dd.MM.yyyy"));
			...
			Если ЗначениеЗаполнено(КвитанцияДата) Тогда
				Текст.ЗаписатьСтроку("КвитанцияДата=" + Формат(КвитанцияДата, "ДФ=dd.MM.yyyy"));
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(КвитанцияВремя) Тогда
				Текст.ЗаписатьСтроку("КвитанцияВремя=" + Формат(КвитанцияВремя, "ДФ=чч:мм:сс"));
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ДатаСписано) Тогда
				Текст.ЗаписатьСтроку("ДатаСписано=" + Формат(ДатаСписано, "ДФ=dd.MM.yyyy"));
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ДатаПоступило) Тогда
				Текст.ЗаписатьСтроку("ДатаПоступило=" + Формат(ДатаПоступило, "ДФ=dd.MM.yyyy"));
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(СрокПлатежа) Тогда
				Текст.ЗаписатьСтроку("СрокПлатежа=" + Формат(СрокПлатежа, "ДФ=dd.MM.yyyy"));
			КонецЕсли;
...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКомандПоУмолчанию = Неопределено, ТолькоВМенюЕще = Ложь, ГруппаДиректБанк = Неопределено) Экспорт
	...
	Если ГруппаДиректБанк <> Неопределено
		И (НЕ ИспользуетсяСервисДиректБанк() ИЛИ НЕ ПравоЧтенияДанных()) Тогда
...
Процедура СформироватьКомандыЭДО(ПолноеИмя, КомандыЭДО, НаправлениеЭД = Неопределено, ТолькоВМенюЕще = Ложь) Экспорт
	...
	ЕстьПравоПросмотраЭД = ПравоЧтенияДанных();
...
Процедура ПрочитатьВыписку1CBankExch(Stmt, ТаблицаВыписок)
	...
	СтрокаВыписки.КонецПериода = XMLЗначение(Тип("Дата"), StmtData.DateTo);
...
Процедура ПрочитатьОперациюВыписки(ЭД, Operation, ТаблицаОпераций)
	...
	НовСтрока.ДатаОперации = XMLЗначение(Тип("Дата"), Operation.Date);
	...
	Если BudgetPaymentInfo <> Неопределено Тогда
		...
		ПлатежВБюджет.Вставить("ПоказательТипа", ЗначениеСвойстваXDTO(PayDataBudget, "PayType"));
