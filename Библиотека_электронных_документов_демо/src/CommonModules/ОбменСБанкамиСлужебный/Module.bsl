...
Функция ОбработатьЭлектронныеДокументы(Параметры, АдресРезультата) Экспорт
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Подписать") Тогда
		...
		Если ТЗ_Сертификатов.Количество() = 0 И ЗначениеЗаполнено(СообщениеОбмена) Тогда
			...
			Если ДоступныеСертификаты[0].Количество Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, НастройкаОбмена);
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Отправить") Тогда
		...
		Если ТЗ_ЭД_КОтправке.Количество() > 0 Тогда
			...
			Отбор = Новый Структура();
			Отбор.Вставить("ТребуетсяПодпись", Истина);
			Отбор.Вставить("ОтправкаЧерезВК", Ложь);
			Отбор.Вставить("ОтправкаЧерезДополнительнуюОбработку", Ложь);
			ВременнаяТЗ = ТЗ_ЭД_КОтправке.Скопировать(Отбор);
...
Процедура ОтметитьИсполненныеПлатежныеДокументы(НастройкаОбмена, ПрограммаБанка, МассивВнешнихИдентификаторов) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		// Если электронный документ не актуален, но в выписке операция отражена, то необходимо вернуть актульный ЭД.
...
Процедура СформироватьИОтправитьЗапросыВыписокСбербанк(Параметры, АдресРезультата) Экспорт
	...
	// Попытка сразу же отправить запросы выписок
	Если НЕ РеквизитыНастройкиОбмена.ИспользуетсяКриптография И ЗначениеЗаполнено(Параметры.ТекущаяСессия) Тогда
		ДанныеДляОтправки = Новый Структура;
...
//
Функция ПодчиненноеСообщениеОбмена(ИсходноеСообщение, ВидЭД = Неопределено) Экспорт
	
	НастройкаОбмена = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсходноеСообщение, "НастройкаОбмена");
...
Процедура ПолучитьНовыеДокументыИзБанка(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	Для Каждого Идентификатор Из МассивИдентификаторов Цикл
		...
		Если Не Результат.Статус Тогда
			ЕстьОтвет = НайтиФайлы(Результат.Тело).Количество() > 0;
			...
			Если ЕстьОтвет Тогда
				ШаблонФайл = НСтр("ru = 'Файл ответа: %1'");
				ТекстОшибки = ТекстОшибки + СтрШаблон(ШаблонФайл, Результат.Тело);
			КонецЕсли;
...
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена.
//  ИмяФайла - Строка - файл, полученный их банка. Файл после обработки удаляется.
...
Процедура ПрочитатьОперациюВыписки(ЭД, Operation, НовыйЭД)
	...
	ДатаОперации = XMLЗначение(Тип("Дата"), Operation.Date);
	...
	Если Дата <> Неопределено Тогда
		Дата = XMLЗначение(Тип("Дата"), Дата);
	КонецЕсли;
	...
	Иначе
		...
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(PayDataBudget, "PayType"), "ПоказательТипа");
	КонецЕсли;
...
Функция СообщенияОбменаТребующиеSMSПодтверждения(МассивСообщений)
	...
	ЗначенияРеквизита = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивСообщений, "Статус");
	
	Для каждого ЭлементКоллекции Из ЗначенияРеквизита Цикл
		
		Если ЭлементКоллекции.Значение = Перечисления.СтатусыОбменСБанками.НеПодтвержден Тогда
