...
Функция ВерсияБиблиотеки() Экспорт
	...
	Возврат Метаданные.Версия; // Автозамена на номер версии АА.ББ.ВВ.ГГ при сборке.
...
Процедура ПриДобавленииПодсистемы(Описание) Экспорт
	...
	Описание.Имя    = "БиблиотекаЭлектронныхДокументовДемо";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ЕстьОбменСКонтрагентами Тогда
		...
		Обработчик.ЧитаемыеОбъекты = "Справочник.МаршрутыПодписания, Справочник.ЭДПрисоединенныеФайлы, Документ.ЭлектронныйДокументИсходящий,
			...
			|Справочник.ПрофилиНастроекЭДО, Справочник.СоглашенияОбИспользованииЭД, Документ.ПакетЭД, РегистрСведений.СостоянияЭД";
		...
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "1.3.10.48";
		Обработчик.РежимВыполнения = "Оперативно";
		Обработчик.ОбщиеДанные      = Ложь;
		Обработчик.НачальноеЗаполнение = Истина;
		Обработчик.Процедура = "ЭлектронноеВзаимодействие.ОбновитьПредопределенныеВидыКонтактнойИнформации";
		Обработчик.Идентификатор = Новый УникальныйИдентификатор("ce7f4be2-4cfb-4b6c-8faf-e0f482bffce0");
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "1.3.10.11";
		Обработчик.Процедура = "Документы._ДемоУдалитьВозвратТоваровОтПокупателя.ОбработатьДанныеДляПереходаНаНовуюВерсию";
		Обработчик.РежимВыполнения = "Отложенно";
		Обработчик.Идентификатор = Новый УникальныйИдентификатор("1b2c2b09-629f-4010-87cb-45379998b01e");
		Обработчик.Комментарий = НСтр("ru = 'Демо: Обмен с контрагентами: изменение параметров документа ""Возврат товаров от покупателя"".
			|Обмен электронными документами с контрагентами временно невозможен.'");
		Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы._ДемоУдалитьВозвратТоваровОтПокупателя.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
		Обработчик.ПроцедураПроверки = "Документы._ДемоУдалитьВозвратТоваровОтПокупателя.ДанныеОбновленыНаНовуюВерсиюПрограммы";
		Обработчик.НачальноеЗаполнение = Ложь;
		Обработчик.БлокируемыеОбъекты = "Документ._ДемоУдалитьВозвратТоваровОтПокупателя, Документ.ПоступлениеТоваровУслуг 
			|Документ.ЭлектронныйДокументИсходящий, Документ.ЭлектронныйДокументВходящий";
		Обработчик.ЧитаемыеОбъекты = "Документ._ДемоУдалитьВозвратТоваровОтПокупателя";
		Обработчик.ИзменяемыеОбъекты = "Документ.ПоступлениеТоваровУслуг, РегистрСведений.СостоянияЭД,
			|Документ.ЭлектронныйДокументИсходящий, Документ.ЭлектронныйДокументВходящий";
		Обработчик.ОчередьОтложеннойОбработки = 3;
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "1.3.10.11";
		Обработчик.Процедура = "Справочники._ДемоУдалитьБанковскиеСчета.ОбработатьДанныеДляПереходаНаНовуюВерсию";
		Обработчик.РежимВыполнения = "Отложенно";
		Обработчик.Идентификатор = Новый УникальныйИдентификатор("f5442a7f-b518-48ca-bfe3-72f8af9582d7");
		Обработчик.Комментарий = НСтр("ru = 'Демо: Обмен с контрагентами: изменение параметров справочника банковские счета.
			|Обмен электронными документами с контрагентами временно невозможен.'");
		Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники._ДемоУдалитьБанковскиеСчета.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
		Обработчик.ПроцедураПроверки = "Справочники._ДемоУдалитьБанковскиеСчета.ДанныеОбновленыНаНовуюВерсиюПрограммы";
		Обработчик.НачальноеЗаполнение = Ложь;
		Обработчик.БлокируемыеОбъекты = "Справочник._ДемоУдалитьБанковскиеСчета, Справочник.БанковскиеСчета,
			|Документ._ДемоЗаказПокупателя, Документ._ДемоЗаказПоставщику, Документ._ДемоОперацииПоБанковскомуСчету,
			|Документ._ДемоОперация, Документ._ДемоОперацияПоЯндексКассе";
		Обработчик.ЧитаемыеОбъекты = "Справочник._ДемоУдалитьБанковскиеСчета";
		Обработчик.ИзменяемыеОбъекты = "Справочник.БанковскиеСчета, Документ._ДемоЗаказПокупателя,
			|Документ._ДемоЗаказПоставщику, Документ._ДемоОперацииПоБанковскомуСчету,
			|Документ._ДемоОперация, Документ._ДемоОперацияПоЯндексКассе";
		Обработчик.ОчередьОтложеннойОбработки = 3;
		
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "1.3.10.11";
		Обработчик.Процедура = "Документы._ДемоПлатежныйДокумент.ОбработатьДанныеДляПереходаНаНовуюВерсию";
		Обработчик.РежимВыполнения = "Отложенно";
		Обработчик.Идентификатор = Новый УникальныйИдентификатор("b05546f6-f5fd-11e7-bfbb-d850e648b5f9");
		Обработчик.Комментарий = НСтр("ru = 'Демо: Обмен с контрагентами: изменение параметров платежных документов.
			|Обмен электронными документами временно невозможен.'");
		Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы._ДемоПлатежныйДокумент.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
		Обработчик.ПроцедураПроверки = "Документ._ДемоПлатежныйДокумент.ДанныеОбновленыНаНовуюВерсиюПрограммы";
		Обработчик.НачальноеЗаполнение = Ложь;
		Обработчик.БлокируемыеОбъекты = "Документ._ДемоПлатежныйДокумент";
		Обработчик.ЧитаемыеОбъекты = "Справочник.БанковскиеСчета";
		Обработчик.ИзменяемыеОбъекты = "Документ._ДемоПлатежныйДокумент";
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "1.3.6.18";
	Обработчик.Процедура = "ЭлектронноеВзаимодействие.ВключитьДемонстрационныеФункциональныеОпции";
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.НачальноеЗаполнение = Истина;
...
Процедура ПриОпределенииРежимаОбновленияДанных(РежимОбновленияДанных, СтандартнаяОбработка) Экспорт
	...
	// Перенос обработчиков после переименования конфигурации
	Если Метаданные.Имя = "БиблиотекаЭлектронныхДокументовДемо" Тогда
		ДанныеОПодсистемах = РегистрыСведений.ВерсииПодсистем.СоздатьНаборЗаписей();
		ДанныеОПодсистемах.Прочитать();
		Для Каждого СтрокаДанных Из ДанныеОПодсистемах Цикл
			Если СтрокаДанных.ИмяПодсистемы = "БиблиотекаЭлектронныхДокументов" Тогда
				СтрокаДанных.ИмяПодсистемы = Метаданные.Имя;
			КонецЕсли;
		КонецЦикла;
		
		Если ДанныеОПодсистемах.Модифицированность() Тогда
			ДанныеОПодсистемах.Записать();
		КонецЕсли;
	КонецЕсли;
