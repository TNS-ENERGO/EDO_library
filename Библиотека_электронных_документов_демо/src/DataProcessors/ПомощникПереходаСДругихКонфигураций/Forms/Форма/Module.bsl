...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.КонфигурацияПриемник = "КЭДО23";
...
Процедура ИмяФайлаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПроверитьУстановкуРасширенияРаботыСФайлами(ДополнительныеПараметры);
...
Процедура ЗаполнитьОписаниеДоступныхКонфигурацийИсточника()
	...
	Если ПустаяСтрока(Объект.КонфигурацияИсточник) Тогда
		...
		Элементы.НадписьПояснение.Заголовок = НСтр("ru = 'Помощник позволит загрузить данные из конфигураций'");
		
		Элементы.НадписьСписокКонфигураций.Видимость = Истина;
		Элементы.НадписьСписокКонфигураций.Заголовок = ТекстОписание;
	Иначе
		...
		Элементы.НадписьПояснение.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Помощник позволит загрузить данные из конфигурации ""%1"".'"), СинонимИсточника);
			
		Элементы.НадписьСписокКонфигураций.Видимость = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеКонфигурацииИсточника()
	...
	ДоступныеКонфигурацииИсточника.Добавить("КЭДО20", НСтр("ru = '1С:Клиент ЭДО Базовая, редакция 2.0'"));
...
Функция ИмяМакетаПравилПоКонфигурацииИсточника(КонфигурацияИсточника, ВидДанных)
	...
	Если КонфигурацияИсточника = "КЭДО20" Тогда
		ИмяМакета = "ПравилаОбменаКлиентЭДОРедакции20";
	КонецЕсли;
...
&НаКлиенте
Процедура ПроверитьУстановкуРасширенияРаботыСФайлами(ДополнительныеПараметры)
...
Функция РезультатЗагрузкиДанныхВДлительнойОперации()
	...
	ПараметрыЗагрузки.Вставить("ЭтапыЗагрузки",      ЭтапыЗагрузки);
	ПараметрыЗагрузки.Вставить("Источник",           Объект.КонфигурацияИсточник);
	ПараметрыЗагрузки.Вставить("Приемник",           Объект.КонфигурацияПриемник);
	ПараметрыЗагрузки.Вставить("БезопаснаяЗагрузка", РазделениеВключено);
	ПараметрыЗагрузки.Вставить("ДанныеДляЗагрузки",  ПолучитьИзВременногоХранилища(АдресДанныхДляЗагрузки));
	ПараметрыЗагрузки.Вставить("АвтотестОбмена",     АвтоТестОбмена);
	...
	Результат = ДлительныеОперации.ВыполнитьВФоне("Обработки.ПомощникПереходаСДругихКонфигураций.ЗагрузитьДанные",
