...
Процедура ИспользоватьРубрикаторБизнесСетьПриИзменении(Элемент)
	...
	ИзменитьИспользованиеРубрикатора1СБизнесСеть();
	Оповестить("Запись_НаборКонстант",, "ИспользоватьРубрикаторБизнесСеть");
...
Процедура ОбновитьДанныеФормы()
	...
	Если Не БизнесСеть.ПравоНастройкиОбменаДокументами() Тогда
		Элементы.ОрганизацияЗаголовок.Доступность = Ложь;
		Элементы.ОрганизацияЗаголовок.Подсказка = "(" + НСтр("ru = 'недостаточно прав'") + ")";
		Элементы.ОрганизацияЗаголовок.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСправа;
	КонецЕсли;
	
	// Сопоставление с рубрикатором 1С:Бизнес-сеть.
	Константа_ИспользоватьРубрикаторБизнесСеть = Константы["ИспользоватьРубрикаторБизнесСеть"].Получить();
	Элементы.СопоставлениеГруппа.Доступность = Константа_ИспользоватьРубрикаторБизнесСеть;
	Элементы.Константа_ИспользоватьРубрикаторБизнесСеть.Доступность =
		ПравоДоступа("Изменение", Метаданные.Константы.ИспользоватьРубрикаторБизнесСеть);
	...
	|ИТОГИ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ОбъектСопоставления)
	|ПО
	|	ОБЩИЕ";
	...
	Если Выборка.Следующий() Тогда
		КоличествоСопоставлено = Выборка.ОбъектСопоставления;
	КонецЕсли;
	...
	// Проверка публикации.
	Запрос = Новый Запрос;
	ТорговыеПредложенияПереопределяемый.ПолучитьТекстЗапросаПубликуемыхТоваров(Запрос.Текст, Ложь, Ложь);
...
&НаСервере
Процедура ИзменитьИспользованиеРубрикатора1СБизнесСеть()
	
	Константы.ИспользоватьРубрикаторБизнесСеть.Установить(Константа_ИспользоватьРубрикаторБизнесСеть);
	ОбновитьДанныеФормы();
