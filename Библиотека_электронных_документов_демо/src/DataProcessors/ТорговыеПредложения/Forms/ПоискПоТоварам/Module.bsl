...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ТорговыеПредложения_ОтправленыЗаказыТоваров"
		И Параметр.Свойство("УникальныйИдентификаторКорзины")
		И Параметр.УникальныйИдентификаторКорзины = УникальныйИдентификаторКорзины Тогда
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ТорговыеПредложения.Форма.ПоискПоТоварам.ОткрытьКарточку");
	...
	Если СтрНайти(Поле.Имя, "_") > 0 Тогда 
		...
		ОчиститьСообщения();
		ОткрытьФорму("Обработка.ТорговыеПредложения.Форма.Карточка", ПараметрыФормы, ЭтотОбъект,
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	// Показывать подсказку, если не заполнена информация по товару и в строке пустое наименование товара (наименование
	// товара заполняется после команды поиска.
	ВидимостьПодсказки = ПустаяСтрока(ИнформацияПоТовару) И Не ПустаяСтрока(ТекущиеДанные.Наименование);
	...
	Для каждого СтрокаПоставщика Из Поставщики Цикл
		...
		Если СтрокаПоставщика.Идентификатор = Идентификатор Тогда
			...
			Если ТекущиеДанные["Пометка_" + СтрокаПоставщика.Идентификатор] = Истина 
				И ПустаяСтрока(ТекущиеДанные["ПредложениеИдентификатор_" + СтрокаПоставщика.Идентификатор]) Тогда
...
Процедура ЗаполнитьПараметрыПоиска(ПараметрыПоиска)
	...
	ТорговыеПредложенияПереопределяемый.ДобавитьСлужебныеКолонкиТовары(СписокЗначение, Истина);
	ЗначениеВРеквизитФормы(СписокЗначение, "Список");
	
	ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыПоиска,
		ТорговыеПредложения.СохраненныеНастройкиПоискаПоТоварам());
	
	ПараметрыПоиска.Вставить("ВалютаКод", ТорговыеПредложения.КодВалюты(Валюта));
	...
	Если РегионПоиска.РежимОтбора <> 0 Тогда
		ПараметрыПоиска.Вставить("РегионРежимОтбора",   РегионПоиска.РежимОтбора);
		ПараметрыПоиска.Вставить("РегионЗначенияПолей", РегионПоиска.ЗначенияПолей);
	КонецЕсли;
...
Процедура ЗагрузитьРезультатыПоиска(АдресРезультата)
	...
	Если Результат <> Неопределено Тогда
		...
		ЗначениеСписок.ЗаполнитьЗначения("",   "ПредложениеИдентификатор");
		ЗначениеСписок.ЗаполнитьЗначения("",   "ПоставщикИдентификатор");
		...
		Для каждого СтрокаРезультата Из ТаблицаРезультата Цикл
			...
			СтрокаСписка["Цена_"                     + Идентификатор] = Макс(0, СтрокаРезультата.Цена);
	...
	Для каждого ЭлементСписка Из Поставщики Цикл
		...
		НовыйЭлемент = Элементы.Добавить("ЛучшаяЦена_" + Идентификатор, Тип("ПолеФормы"),
		...
		НовыйЭлемент.ПутьКДанным = "Список.ЛучшаяЦена_" + Идентификатор;
		НовыйЭлемент.РастягиватьПоГоризонтали = Ложь;
		...
		НовыйЭлемент = Элементы.Добавить("ЛучшийСрок_" + Идентификатор, Тип("ПолеФормы"),
		Элементы["ГруппаПоставка_" + Идентификатор]);
		НовыйЭлемент.ОтображатьВШапке = Ложь;
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеКартинки;
		НовыйЭлемент.КартинкаЗначений = БиблиотекаКартинок.ЛучшиеТорговыеПредложения;
		НовыйЭлемент.ПутьКДанным = "Список.ЛучшийСрок_" + Идентификатор;
		НовыйЭлемент.РастягиватьПоГоризонтали = Ложь;
		НовыйЭлемент.Ширина = 1;
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Список.Цена_" + Идентификатор);
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно;
		ОтборЭлемента.ПравоеЗначение = 0; // Проверка на 0 или -1.
		ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Список.ПредложениеИдентификатор_" + Идентификатор);
		...
		ГруппаОтбора.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ;
		ОтборЭлемента = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Список.СрокПоставки_" + Идентификатор);
		...
		// Условное оформление. Очистка срока поставки если он не задан (-1).
		ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
		ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы["СрокПоставки_" + Идентификатор].Имя);
		ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
...
Процедура ОбновитьПоказателиПоставщиков(РежимСнятияПометок = Ложь, ИдентификаторыПоставщиков = Неопределено)
	...
	Для каждого СтрокаПоставщики Из Поставщики Цикл
		...
		Для каждого СтрокаСписка Из Список Цикл
			...
			Если ЗначениеЗаполнено(СтрокаСписка["ПредложениеИдентификатор_" + СтрокаПоставщики.Идентификатор]) Тогда
				СтрокаПоставщики.Охват = СтрокаПоставщики.Охват + 1;
			КонецЕсли;
...
Функция РеквизитыНоменклатуры(Знач Номенклатура)
	...
	Реквизиты = ТорговыеПредложения.ОписаниеРеквизитовНоменклатуры();
...
Процедура ВыборРегионаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("РегионРежимОтбора") Тогда
		...
		СохранитьРегионПоиска(РегионПоиска);
...
Процедура ОбновитьЗаголовокРегиона(Объект)
	...
	РегионЗаголовок = "";
	Если Объект.РегионПоиска.РежимОтбора = 0 Тогда
		РегионЗаголовок = НСтр("ru = 'Без отбора по регионам'");
	ИначеЕсли Объект.РегионПоиска.РежимОтбора = 1 Тогда
		Отказ = Ложь;
		...
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(Объект.РегионПоиска.ЗначенияПолей) Тогда
		Отказ = Ложь;
		Регион = ПоследнийУровеньКонтактнойИнформации(Объект.РегионПоиска.ЗначенияПолей, Отказ);
		Если Отказ Тогда
...
&НаКлиенте
Процедура НайтиПредложенияНачало()
	
	ОчиститьСообщения();
	
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ТорговыеПредложения.Форма.ПоискПоТоварам.НайтиПредложения");
	...
	ДлительнаяОперацияЗавершение = Новый ОписаниеОповещения("НайтиПредложенияВФонеЗавершение",
...
&НаСервереБезКонтекста
Процедура СохранитьРегионПоиска(РегионПоиска)
