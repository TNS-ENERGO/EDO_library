...
Процедура ОбработчикЗагрузитьТорговыеПредложения()
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ТорговыеПредложения.Форма.ПоискПоОтборам.НайтиПредложения");
...
Процедура ОбработчикЗаполнитьНавигациюКатегорийИЗагрузитьТорговыеПредложения()
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ТорговыеПредложения.Форма.ПоискПоОтборам.ЗаполнитьНавигацию");
...
Процедура ЗаполнитьНавигациюКатегорий(Идентификатор, ОчиститьСтруктуру = Ложь, Отказ = Ложь)
	...
	Если КореньСОднойГруппой ИЛИ ОтличиеПодчиненных Тогда
		...
		Если Идентификатор = "" ИЛИ Идентификатор = Неопределено Тогда
			...
			// Добавление служебной рубрики.
			НоваяСтрока = СтрокаДобавления.Строки.Добавить();
			НоваяСтрока.Идентификатор = "0";
			НоваяСтрока.Представление = НСтр("ru = 'Разное'");
			НоваяСтрока.КоличествоПодчиненных = 0;
		...
		ИначеЕсли ЗначениеЗаполнено(Идентификатор)
			...
			Если Идентификатор = "0" Тогда
				// Добавление служебной рубрики.
				СтрокаДобавления = СтрокаДобавления.Строки.Добавить();
				СтрокаДобавления.Идентификатор = "0";
				СтрокаДобавления.Представление = НСтр("ru = 'Разное'");
				СтрокаДобавления.КоличествоПодчиненных = 0;
			КонецЕсли;
			
		ИначеЕсли Идентификатор = "0" Тогда
			// Служебная рубрика, пропускаем изменение.
		Иначе
...
Функция СформироватьПараметрыПоиска()
	...
	Если ЗначениеЗаполнено(ТекущаяКатегория) ИЛИ ТекущаяКатегория = "0" Тогда
...
Процедура ДозаполнитьПараметрыПроцедуры(ПараметрыПоиска)
	...
	ПараметрыПоиска.Вставить("КодВалюты", ТорговыеПредложения.КодВалюты(ПараметрыПоиска.Валюта));
...
Процедура УстановитьУсловноеОформление()
	...
	// Надпись "в наличии" (срок поставки).
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	...
	// Очистка незаполненного срока поставки.
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СписокСрокПоставки.Имя);
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
...
Процедура ОткрытьКарточкуТорговогоПредложения()
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ТорговыеПредложения.Форма.ПоискПоОтборам.ОткрытьКарточку");
	...
	ИначеЕсли ИмяКолонки = "СписокГиперссылкаОчистить" Тогда
	...
	Иначе
		...
		ОткрытьФорму("Обработка.ТорговыеПредложения.Форма.Карточка", ПараметрыОткрытия, ЭтотОбъект,
...
Процедура ОбновитьЗаголовокРегиона(Объект)
	...
	Если Объект.ТребуетсяРегистрацияОрганизации Тогда
		Объект.РегионПоиска.РежимОтбора = 0;
	КонецЕсли;
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.РегионПоиска.ЗначенияПолей) Тогда
		Отказ = Ложь;
		Объект.РегионЗаголовок = ПоследнийУровеньКонтактнойИнформации(Объект.РегионПоиска.ЗначенияПолей, Отказ);
		Если Отказ Тогда
			Объект.РегионЗаголовок = НСтр("ru = 'Без отбора по регионам'");
		КонецЕсли;
	Иначе
