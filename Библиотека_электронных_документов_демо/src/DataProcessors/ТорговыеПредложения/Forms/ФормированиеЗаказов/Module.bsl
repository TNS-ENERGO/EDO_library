...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СпособыДоставкиСамовывоз = Новый СписокЗначений;
	...
	Элементы.ЗаказыКОформлениюСпособДоставки.СписокВыбора.ЗагрузитьЗначения(СпособыДоставки.ВыгрузитьЗначения());
	...
	Если РежимЗапросаЦен Тогда
		...
		Элементы.ЗаказыКОформлениюСуммаСНДС.Видимость =      Ложь;
		Элементы.ЗаказыКОформлениюСпособДоставки.Видимость = Ложь;
		Элементы.ЗаказыКОформлениюАдресДоставки.Видимость =  Ложь;
		...
		Элементы.ТоварыХарактеристикаНоменклатуры.Видимость = Ложь;
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "ТоварыПоставщикНаименование" Тогда
		...
		ПараметрыОткрытия.ИНН = ТекущиеДанные.ПоставщикИНН;
		ПараметрыОткрытия.КПП = ТекущиеДанные.ПоставщикКПП;
...
Процедура ПересчитатьИтоги(Форма)
	...
	Форма.СуммаВсего = Форма.ЗаказыКОформлению.Итог("СуммаСНДС");
...
Процедура ИзменитьКоличествоВКорзинеПоИдентификатору(ПредложениеИдентификатор, Количество, ЭтоРазница = Ложь)
	...
	Иначе
		...
		Отказ = Ложь;
		Предложение = ТорговыеПредложения.ПрочитатьТорговоеПредложение(ПредложениеИдентификатор, Валюта, , Отказ);
		...
		СтрокаТовары.ПоставщикКПП    = БизнесСетьКлиентСервер.ЗначениеИдентификатора(Предложение.vendor.kpp);
...
Процедура ЗаполнитьНоменклатуруПоНоменклатуреПоставщика(СтрокаТовары = Неопределено)
	...
	Если НоменклатураПоставщиков.Количество() Тогда
		...
		ТаблицаНоменклатурыПоставщиков = Новый ТаблицаЗначений;
		ТаблицаНоменклатурыПоставщиков.Колонки.Добавить("НоменклатураПоставщика");
		ТаблицаНоменклатурыПоставщиков.Колонки.Добавить("Номенклатура");
		ТаблицаНоменклатурыПоставщиков.Колонки.Добавить("ИспользованиеХарактеристик");
		ТаблицаНоменклатурыПоставщиков.Колонки.Добавить("Упаковка");
		ТаблицаНоменклатурыПоставщиков.Колонки.Добавить("ХарактеристикаНоменклатуры");
		ТаблицаНоменклатурыПоставщиков.Колонки.Добавить("ЕдиницаИзмеренияНоменклатуры");
		
		ТорговыеПредложенияПереопределяемый.ПолучитьРеквизитыНоменклатурыПоставщика(НоменклатураПоставщиков, ТаблицаНоменклатурыПоставщиков);
	...
	Иначе
		...
		// Может быть два типа Неопределено и ссылка.
		Элементы.ТоварыГруппаНоменклатура.Видимость = Макс(
			СтрокиТоваров.НайтиСтроки(Новый Структура("Номенклатура", Неопределено)).Количество(),
			СтрокиТоваров.НайтиСтроки(Новый Структура("Номенклатура", Справочники[ИмяСправочникаНоменклатура].ПустаяСсылка())).Количество(),
...
Процедура ДлительнаяОперацияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не Отказ И Статус = "Выполнено" Тогда
		...
		Если ТекущаяСтраница = Элементы.СтраницаПодготовкаЗаказов
			...
			Если ЗначениеЗаполнено(Результат.АдресРезультата)
				...
				Отказ = ПрочитатьДанныеИзБезопасногоХранилища();
...
Процедура ЗаполнитьПараметрыФормы()
	...
	ЗаголовокСумма = СтрШаблон(НСтр("ru = 'Сумма %1'"), ?(Валюта.Пустая(), "", "("+ Валюта +")"));
	...
	Если Параметры.Свойство("АдресТоваровВХранилище", АдресТоваровВХранилище) И ЗначениеЗаполнено(Параметры.АдресТоваровВХранилище) Тогда
		...
		Иначе
			Для каждого ЭлементКоллекции Из ТаблицаТоваров Цикл
				ЗаполнитьЗначенияСвойств(Товары.Добавить(), ЭлементКоллекции,, "Цена, Сумма");
			КонецЦикла;
		КонецЕсли;
	...
	ИначеЕсли Параметры.Свойство("СписокТоваров") Тогда
		...
		Для каждого СоставПозиции Из Параметры.СписокТоваров Цикл
			СтрокаТовары = Товары.Добавить();
			Если РежимЗапросаЦен Тогда
				ЗаполнитьЗначенияСвойств(СтрокаТовары, СоставПозиции,, "Цена, Сумма");
			Иначе
