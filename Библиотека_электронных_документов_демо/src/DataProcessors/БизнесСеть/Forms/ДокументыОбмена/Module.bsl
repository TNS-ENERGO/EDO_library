...
Процедура ОбновитьСписок(Отказ)
	...
	Результат = БизнесСетьВызовСервера.ПолучитьВходящиеДокументы(ПараметрыЗапроса, Отказ);
	Список.Очистить();
	...
	Для каждого ЗначениеМассива Из Результат Цикл
		...
		Если РежимИсходящихДокументов Тогда
			...
			НоваяСтрока.КонтрагентКПП  = БизнесСетьКлиентСервер.ЗначениеИдентификатора(
				ЗначениеМассива.destinationOrganization.Kpp);
			НоваяСтрока.ОрганизацияИНН = ЗначениеМассива.sourceOrganization.Inn;
			НоваяСтрока.ОрганизацияКПП = БизнесСетьКлиентСервер.ЗначениеИдентификатора(
