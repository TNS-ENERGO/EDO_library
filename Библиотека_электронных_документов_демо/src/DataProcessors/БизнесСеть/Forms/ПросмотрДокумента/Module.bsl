	...
	Если ТипЗнч(ДанныеЭД) = Тип("ТабличныйДокумент") Тогда
		...
		Если ЭтоВходящийДокумент Тогда
			...
			Для каждого ЭлементСписка Из СписокТипов Цикл
				...
				ТекЭлемент.Значение = ЭлементСписка.Представление;
				
				// Если реквизит ДокументИБ еще не заполнен и зачитано первое по списку значение, то заполним имеющимися данными.
				Если НЕ ЗначениеЗаполнено(Ссылка) И СписокТипов.Индекс(ЭлементСписка) = 0 Тогда
					ТипОбъекта = ЭлементСписка.Представление;
					Ссылка = ЭлементСписка.Значение;
					ИмяОбъектаМетаданных = ЭлементСписка.Значение.Метаданные().ПолноеИмя();
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(ЗаполняемыйДокумент)
					И ТипЗнч(ЗаполняемыйДокумент) = ТипЗнч(ЭлементСписка.Значение) Тогда
					ТипОбъекта = ЭлементСписка.Представление;
