...
Процедура КонтрагентНажатие(Элемент, СтандартнаяОбработка)
	...
	БизнесСетьКлиент.ОткрытьПрофильУчастника(Контрагент);
...
Процедура ОтправитьДокументыНаСервере(Отказ)
	...
	Если Не Отказ Тогда
		...
		Если НаправитьУведомление Тогда
			Результат = БизнесСетьВызовСервера.ОтправитьУведомлениеОбОтправке(Контрагент, МассивИдентификаторов,
...
Процедура ОбновитьСписокИсторииОтправки(Отказ)
	...
	ПараметрыМетода.Вставить("МассивСсылокНаОбъект", МассивСсылокНаОбъект);
	ПараметрыМетода.Вставить("РежимВходящихДокументов", Ложь);
	ПараметрыМетода.Вставить("withData", "false");
	...
	Если ТипЗнч(ДанныеСервиса) = Тип("Массив") Тогда
		...
		Для каждого ЗначениеМассива Из ДанныеСервиса Цикл
			
			НоваяСтрока = СписокИстории.Добавить();
			НоваяСтрока.Дата = БизнесСетьКлиентСервер.ДатаИзUnixTime(ЗначениеМассива.sentDate);
			Если ВРег(ЗначениеМассива.deliveryStatus) = "SENT" Тогда
				НоваяСтрока.Статус = НСтр("ru = 'Отправлен'");
			ИначеЕсли ВРег(ЗначениеМассива.deliveryStatus) = "DELIVERED" Тогда
				НоваяСтрока.Статус = НСтр("ru = 'Доставлен'");
			ИначеЕсли ВРег(ЗначениеМассива.deliveryStatus) = "REJECTED" Тогда
				НоваяСтрока.Статус = НСтр("ru = 'Отклонен'");
			КонецЕсли; 
			
			НоваяСтрока.Наименование  = ЗначениеМассива.documentTitle;
			НоваяСтрока.Идентификатор = ЗначениеМассива.id;
			Если ЗначениеЗаполнено(ЗначениеМассива.receivedDate) Тогда
				НоваяСтрока.ДатаДоставки = БизнесСетьКлиентСервер.ДатаИзUnixTime(ЗначениеМассива.receivedDate);
			КонецЕсли; 
			
			// Получение владельца из ТаблицаДанных.
			СтрокаДанных = ТаблицаДанных.НайтиСтроки(Новый Структура("УникальныйИдентификатор", ЗначениеМассива.documentGuid));
			НоваяСтрока.ВладелецЭД = СтрокаДанных[0].ВладелецЭД;
			
			НоваяСтрока.Информация    = ЗначениеМассива.info;
			НоваяСтрока.Получатель    = ЗначениеМассива.destinationOrganization.title;
			НоваяСтрока.КонтрагентИНН = ЗначениеМассива.destinationOrganization.inn;
			НоваяСтрока.КонтрагентКПП = ЗначениеМассива.destinationOrganization.kpp;
