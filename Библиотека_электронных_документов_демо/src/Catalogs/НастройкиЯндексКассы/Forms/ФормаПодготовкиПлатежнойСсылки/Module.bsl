...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НастройкаЯндексКассы = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не обнаружены настройки подключения к Яндекс.Кассе'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВариантДоставкиЧека(КонтактныеДанныеЭлектронногоЧека)
	...
	Иначе
		...
		СтрокаТелефона = ТелефонВСтрокуВнутр(КонтактныеДанныеЭлектронногоЧека);
		ДоставкаЧекаТелефон = ТелефонИзСтрокиВнутр(СтрокаТелефона);
		ВариантДоставкиЧека = "Телефон";
...
&НаКлиентеНаСервереБезКонтекста 
Функция ТелефонИзСтрокиВнутр(Знач СтрокаТелефона)
	
	Телефон = "+7";
	
	Если ЗначениеЗаполнено(СтрокаТелефона) Тогда
		
		Код = Лев(СтрокаТелефона, 3);
		ТелефонБезКода = "";
		
		Если СтрДлина(СтрокаТелефона) > 3 Тогда;
			ТелефонБезКода = Сред(СтрокаТелефона, 4);
		КонецЕсли;
		
		Телефон = Телефон + "(" + Код + ")" + ТелефонБезКода;
