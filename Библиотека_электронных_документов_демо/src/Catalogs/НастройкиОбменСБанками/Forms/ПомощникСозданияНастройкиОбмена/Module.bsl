	...
	Если ТекстПервойОшибки <> Неопределено Тогда
		...
		Исключение
			Если НЕ ПробнаяОперация Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстПервойОшибки);
			КонецЕсли;
		КонецПопытки;
	КонецЕсли;
...
Процедура ПослеСкачиванияВнешнейКомпонентыСбербанка(Результат, ДополнительныеПараметры) Экспорт
	...
	Назначение = "НастройкаОбмена";
	ОбменСБанкамиСлужебныйКлиент.ВыполнитьБазовуюАутентификациюСбербанк(
		Оповещение, ИмяВнешнегоМодуля, УникальныйИдентификатор, НастройкаОбмена, ЛогинПароль, Назначение);
