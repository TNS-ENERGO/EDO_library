...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоПрямойОбмен = ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(Объект.СпособОбменаЭД);
...
Процедура ИдентификаторОрганизацииОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Не КонтролироватьСодержимоеНастроекЭД Тогда
		ТекстВопроса = ТекстВопросаОбИзмененииСвязанныхНастроекЭДО();
...
Процедура ИсходящиеДокументыПриИзменении(Элемент)
	...
	Если Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД")
		ИЛИ НЕ ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
	...
	Если (Элемент.ТекущиеДанные.ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура")
		...
		И ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(Объект.СпособОбменаЭД) Тогда
	...
	Если ВРег(Элемент.ТекущийЭлемент.Имя) = ВРег("ИсходящиеДокументыДокументУчета") Тогда
		...
		ОбновитьФлагиРегламентаПоСтроке(Элемент.ТекущиеДанные, Объект.СпособОбменаЭД, Объект.ИспользоватьУПД, Объект.ИспользоватьУКД);
...
Процедура ИсходящиеДокументыФормироватьПриИзменении(Элемент)
	...
	Если Не КонтролироватьСодержимоеНастроекЭД Тогда
		ТекстВопроса = ТекстВопросаОбИзмененииСвязанныхНастроекЭДО();
		ПараметрыОповещения = Новый Структура;
		ПараметрыОповещения.Вставить("Формировать", Элементы.ИсходящиеДокументы.ТекущиеДанные.Формировать);
		ПараметрыОповещения.Вставить("ИДСтроки", Элементы.ИсходящиеДокументы.ТекущаяСтрока);
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"РазрешитьРедактированиеИсходящиеДокументыФормироватьЗавершить", ЭтотОбъект, ПараметрыОповещения);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
	КонецЕсли;
...
&НаКлиенте
Функция ТекстВопросаОбИзмененииСвязанныхНастроекЭДО()
...
Процедура ОбновитьФлагиРегламентаПоСтроке(Строка, СпособОбмена, ИспользоватьУПД, ИспользоватьУКД)
	
	// Если флаг использования установлен, нужно актуализировать флаги ответной подписи и извещения.
	Если Не ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(СпособОбмена)
		И Строка.Формировать Тогда
		
		НастройкиРегламентаЭДО = ОбменСКонтрагентамиСлужебныйКлиентСервер.НастройкиРегламентаЭДО(Строка.ИсходящийДокумент,
			Строка.ВерсияФормата, СпособОбмена);
...
Процедура УстановитьВидимостьЭлементов(ОбновитьРегламентЭДО = Ложь)
	...
	Элементы.ГруппаУПДУКД.Видимость = Не ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(Объект.СпособОбменаЭД);
	Элементы.ТекстПриглашений.Видимость = Не ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(Объект.СпособОбменаЭД);
	...
	Элементы.НачальнаяДатаЗапросаДанныхУОператора.Видимость = Не ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(Объект.СпособОбменаЭД);
...
Процедура ПодготовитьДанныеПоРегистрационнойИнформации(РеквизитыПакета, ДвоичныеДанныеДляОператораЭДО)
	...
	РеквизитыПакета.Вставить("Телефон",          "");
	...
	Макет = ОбменСКонтрагентамиВнутренний.МакетЭлектронногоДокумента("РегистрационнаяИнформация");
	...
	ЭтоФизЛицо = Ложь;
	ОбменСКонтрагентамиПереопределяемый.ЭтоФизЛицо(Объект.Организация, ЭтоФизЛицо);
...
Процедура РазрешитьРедактированиеДополнительныхНастроекЗавершить(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	НастройкиРегламентаЭДО = ОбменСКонтрагентамиСлужебныйКлиентСервер.НастройкиРегламентаЭДО(ДанныеСтроки.ИсходящийДокумент, ДанныеСтроки.ВерсияФормата, Объект.СпособОбменаЭД);
...
Процедура ИзменитьФлагУПДУКД()
	...
	Для каждого Строка Из Объект.ИсходящиеДокументы Цикл
		...
		ОбновитьФлагиРегламентаПоСтроке(Строка, Объект.СпособОбменаЭД, Объект.ИспользоватьУПД, Объект.ИспользоватьУКД);
		ОбменСКонтрагентамиСлужебныйКлиентСервер.ЗаполнитьПредставлениеРегламентаЭДОВСтроке(Строка);
...
&НаКлиенте
Процедура РазрешитьРедактированиеИсходящиеДокументыФормироватьЗавершить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		КонтролироватьСодержимоеНастроекЭД = Истина;
	Иначе
		ИзменитьФлагСтрокиФормировать(ДополнительныеПараметры);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ИзменитьФлагСтрокиФормировать(ПараметрыСтроки)
	
	Строка = Объект.ИсходящиеДокументы[ПараметрыСтроки.ИДСтроки];
	Строка.Формировать = Не ПараметрыСтроки.Формировать;
