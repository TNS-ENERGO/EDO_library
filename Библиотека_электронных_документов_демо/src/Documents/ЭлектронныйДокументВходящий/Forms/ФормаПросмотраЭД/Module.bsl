...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	СвойстваЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Ссылка,
		"ВидЭД, ПрофильНастроекЭДО, НастройкаЭДО, ПричинаОтклонения, ТребуетсяПодтверждение");
	...
	Если Не ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(
	...
	Если СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
		...
		Если НЕ ЭтоСлужебный И НЕ СвойстваЭД.ТребуетсяПодтверждение Тогда
	...
	ИначеЕсли СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий Тогда
		...
		Если ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(СвойстваЭД.ПрофильНастроекЭДО.СпособОбменаЭД) Тогда
...
Функция ОбменЧерезОператора(СвойстваЭД)
	...
	Выборка = Результат.Выбрать();
	Выборка.Следующий();
	Если Не ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(Выборка.СпособОбменаЭД) Тогда
		Возврат Истина
	Иначе
		Возврат Ложь;
	КонецЕсли;
