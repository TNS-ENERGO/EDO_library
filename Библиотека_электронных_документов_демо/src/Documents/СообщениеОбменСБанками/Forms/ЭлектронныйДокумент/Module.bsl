...
Функция ЭтоСлужебныйДокумент(ВидЭД)
	...
	ВидыСлужебныхДокументов.Вставить(Перечисления.ВидыЭДОбменСБанками.ЗапросНочнойВыписки, Истина);
...
Процедура ПолучитьФайлДляТехническойПоддержки(Знач СообщениеОбмена, Знач ВидЭД, Знач ПрограммаБанка, Знач УникальныйИдентификатор, СсылкаНаФайл, ИмяФайла)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ДанныеСертификата) = Тип("ДвоичныеДанные") Тогда
			ДанныеСертификата.Записать(ВремКаталог + ИмяФайлаДанных);
		Иначе // строка
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(ДанныеСертификата);
			ТекстовыйДокумент.Записать(ВремКаталог + ИмяФайлаДанных);
		КонецЕсли;
		ФайлАрхива.Добавить(ВремКаталог + ИмяФайлаДанных);
	КонецЦикла;
